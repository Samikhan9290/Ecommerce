            ...
            if($result[0]->is_one_time==1){
                ...
                if($min_order_amt>0){
                    ...
                    if($min_order_amt<$totalPrice){
                        $status="success";
                        $msg="2Coupon code added";
                    }
                    else{
                        $status="error";
                        $msg="Cart amount must me greater than $totalPrice";
                    }
                }
                else{
                    $status="success";
                    $msg="3Coupon code added";
                }
            }
//                $status="success";
//                $msg="4Coupon code added";
        }
        else{
            $status="error";
...
function getAvalaibleQty($product_id,$attr_id){
    $result=DB::table('order_detail')
        ...
        ->where(['order_detail.product_attr_id'=>$attr_id])
        ->select('order_detail.qty','products_attr.qty as pqty')
        ->get();
    return $result;
